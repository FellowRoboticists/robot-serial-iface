...
module.exports = (function () {
  ...
  RobotSerialInterface.prototype.emitBatch = function () {
    this.__emitBatch = true
  }
        ...
        while (sensorValues) {
          for (let i = 0; i < sensorValues.length; i++) {
            if (rsi.__meetsThreshold(sensorValues[i].name, sensorValues[i].value)) {
              eventData[sensorValues[i].name] = sensorValues[i].value
            }
          }
